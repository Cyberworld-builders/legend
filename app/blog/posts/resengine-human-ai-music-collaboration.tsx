...
export default function Post() {
  return (
    <PostLayout meta={metadata}>
      ...
      <p>DrumNote is the capability to generate standard musical notation for drum tracks. Take a music track, a song or a sample of a song, separate it into stems so we can isolate the drum track, then analyze just the drums and find the kick, snare, and cymbals. Ideally we want it all, toms, whatever, but different genres and different producers use different kinds of percussion, so let&apos;s keep it simple. Start with what&apos;s viable, meaningful, and useful. Get the core rhythm of the beat. Kick drum, snare, cymbals. Those three things. That&apos;s a huge win and a viable solution right there.</p>

      <p>We can iterate as the code matures, as we gather more data, and as the foundational AI models advance. But the starting point is those three elements.</p>

      <p>At its core, though, DrumNote is really just CRUD functionality. Create, read, update, and delete notation objects. That&apos;s the foundation. The ability to generate notation from an audio file is more like an import feature. There&apos;d probably be part of the interface that can access your microphone and sample on the spot, but the bedrock is CRUD on notation objects.</p>

      <h2>NotePulse: Notation to Audio</h2>

      <p>NotePulse is the reverse of DrumNote. It takes drum notation and generates a beat. So where DrumNote can export to NotePulse, NotePulse has its own CRUD functionality around a different data object: an actual audio file, a rhythm track that you can play and listen to. I don&apos;t have a clean word for that data object yet, but that&apos;s what it is.</p>

      <h2>Music as Code</h2>

      <p>This is the bigger idea. We want to take classical standard music notation as a bedrock and expand it into a rich, fine-grained data object that explains everything about the music. Take notation and blow it up into something deeper.</p>

      <p>It feels like this breaks into two main categories: analyzing music from a technical perspective, and analyzing music from a more human, emotional, expressive storytelling perspective. The latter becomes more relevant as we move beyond drums into other parts of the music.</p>
      ...
      <p>All of these data objects are going to go under the same schema. I&apos;ll probably call it CyberWorld Media Studios. It&apos;ll use CyberWorld login for identities, have its own profile. Each one of these products could be standalone, but I want to first develop them as scripts that I can run and define a manual workflow before I start building any software at all.</p>
      ...
      <p>We can iterate on all of this as the foundational models improve and as we gather more data. The point is to start with something viable, start with drums, and build from there.</p>
    </PostLayout>
  );
}